...
before ......
    ...
    try:
        async with db.cursor() as c:
            ...
            await c.execute(
                """CREATE TABLE IF NOT EXISTS question(Id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, topic varchar(256), level int, result int, priority int)"""
            )
            ##########################
            await db.commit()
        # await c.close()
...
async def post_new_blog(db, uid, title, contents, utime=0.0):
    try:
        async with db.cursor() as cur:
            ...
            await cur.execute(
                "INSERT INTO blog (uId,title,contents,utime) VALUES (%s,%s,%s,%s)",
                (uid, title, contents, utime),
            )
            await db.commit()
            # await cur.close()
...
async def post_update_blog(db, id, title, contents):
    try:
        async with db.cursor() as cur:
            sql = "UPDATE blog SET title = '{}', contents = '{}' WHERE Id = {}".format(
                title, contents, int(id)
            )
            logger.debug(sql)
            await cur.execute(
                "UPDATE blog SET title =%s, contents =%s WHERE Id = %s",
                (title, contents, int(id)),
            )
            await db.commit()
...
async def create_new_user(db, name, email, hash_password):
    ...
    try:
        async with db.cursor() as cur:
            ...
            r = await cur.execute(
                "INSERT INTO author (email, nickname, passwd) VALUES (%s,%s,%s)",
                (email, name, hash_password),
            )
            ...
            await db.commit()
            await cur.execute(
                "SELECT Id, nickname, email FROM author WHERE email = %s", email
            )
...
async def get_user_by_email(db, email):
    try:
        async with db.cursor() as cur:
            sql = "SELECT Id, nickname , email, passwd FROM author WHERE email = '{}'".format(
                email
            )
            logger.debug(sql)
...
async def get_last_users(db, counts=10):
    try:
        ...
        async with db.cursor() as cur:
            await cur.execute(
                "SELECT Id, nickname, email FROM author ORDER BY Id DESC LIMIT 0,%s",
                (counts),
            )
            rets = await cur.fetchall()
            if len(rets) > 0:
        ...
        async with db.cursor() as cur:
            ...
            if len(rets) > 0:
                for r in rets:
                    blogs.append(
                        dbmodel.Blog(
                            int(r[0]), int(r[1]), str(r[2]), "", r[3], r[4], str(r[5])
                        )
                    )
            # cur.close()
    except Exception as e:
...
async def question_db_init(db):
    ...
    for i in range(100):
        for j in range(100):
            ...
            re1 = i + j
            s1 = "{} + {}".format(i, j)
            sql1 = "insert into question(topic, level, result, priority) values ('{}',{},{},{})".format(
                s1, 1, re1, 1
            )
            print(sql1)
            ...
            s2 = "{} - {}".format(i, j)
            if re2 > 0:
                sql2 = "insert into question(topic, level, result, priority) values ('{}',{},{},{})".format(
                    s2, 1, re2, 1
                )
                print(sql2)
